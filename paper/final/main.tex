...
\hypersetup{
    pdftitle={Transform Ordering Dominates Selection in Image Preprocessing Pipelines for Skin Disease Classification},
...
% TITLE AND AUTHORS
% ===================================================================
\title{Transform Ordering Dominates Selection in Image Preprocessing Pipelines for Skin Disease Classification}
...
The experimental dataset was constructed by combining dermoscopic images of skin lesions from the HAM10000 collection~\cite{DBLP:journals/corr/abs-1803-10417} with images of healthy skin from the Healthy Skin dataset~\cite{megpaulson_healthy_skin}, forming a binary classification task (diseased versus healthy). Images were resized to a uniform spatial resolution prior to model ingestion.
...
\textbf{Intensity Normalization (N)} applies an affine transformation $x' = (x - \mu_t) / \sigma_t$ to rescale pixel values toward a target mean $\mu_t = 0.4$ and standard deviation $\sigma_t = 0.2$. This standardization reduces inter-sample variability in lighting and contrast, allowing the network optimizer to converge more efficiently~\cite{irjet}.
...
\textbf{Non-Local Means Denoising (D)} suppresses acquisition noise by computing weighted averages over similar image patches, using a template window of 5\,pixels and a search window of 19\,pixels~\cite{MAFI2019236, elad2023imagedenoisingdeeplearning}. Unlike local filters, non-local means preserves repetitive patterns and edge structures characteristic of dermatological lesions.
...
The central methodological contribution of this study is the exhaustive evaluation of all possible preprocessing pipelines constructible from the four selected transforms. At each pipeline length $k$ (from 1 to 4), every ordered permutation of $k$ distinct transforms drawn from the set $\{\text{E}, \text{N}, \text{D}, \text{CS}\}$ was evaluated independently. This yields $P(4,1) + P(4,2) + P(4,3) + P(4,4) = 4 + 12 + 24 + 24 = 64$ non-baseline pipelines, plus the unprocessed baseline (length zero), for a total of 65 evaluated configurations.
...
\begin{table*}[!t]
\caption{Pipeline Performance Results ($\varepsilon_0 = 88.5\%$). Pipelines are grouped by length; $\alpha$ is the accuracy gain in percentage points and $\alpha_w$ is the cost-adjusted accuracy gain as defined in~\eqref{eq:alpha_w}.\label{tab:results}}
\centering
\footnotesize
...
% Placeholder: box-plot / violin of alpha by pipeline length
\begin{figure}[!t]
\centering
\fbox{\rule{0pt}{2in}\rule{0.9\columnwidth}{0pt}}
...
% Placeholder: variance decomposition figure
\begin{figure}[!t]
\centering
\fbox{\rule{0pt}{2in}\rule{0.9\columnwidth}{0pt}}
...
% Placeholder: positional gradients figure
\begin{figure}[!t]
\centering
\fbox{\rule{0pt}{2in}\rule{0.9\columnwidth}{0pt}}
