...
\hypersetup{
  pdftitle={Significance of Transform Ordering and Selection in Image Preprocessing for CNN-Based Skin Disease Classification},
...
% TITLE AND AUTHORS
% ===================================================================
\title{Significance of Transform Ordering and Selection in Image Preprocessing for CNN-Based Skin Disease Classification}
...
\textbf{Gradient-weighted Class Activation Mapping} (Grad-CAM)~\cite{8237336} was applied to the final convolutional layer of each trained model to visualize the spatial regions most influential to the classifier's prediction. For each of the 65 pipeline configurations, Grad-CAM saliency maps were generated for six test images sampled at three confidence strata per class: a high-confidence correct prediction, a decision-boundary case (confidence $\approx 0.5$), and a near-misclassification case. Both positive-class (diseased) and negative-class (healthy) activation overlays were produced for each image, yielding 12 visualizations per pipeline and 780 Grad-CAM images per confidence regime.
...
Table~\ref{tab:results} presents the accuracy gain ($\alpha$) and cost-adjusted accuracy gain ($\alpha_w$) for all 65 pipeline configurations. The distribution of outcomes across the 64 non-baseline pipelines is strikingly asymmetric: 15 of 64 (23.4\%) of pipelines achieve positive $\alpha$ (accuracy improvement over the unprocessed baseline), while the remaining 77\% degrade performance.

The best-performing pipeline is CS$\to$E$\to$N, achieving $\alpha = +1.57$\,pp with $\alpha_w = 1.86$. Two other length-three configurations, E$\to$N$\to$D ($\alpha = +1.16$\,pp) and D$\to$E$\to$N / E$\to$D$\to$N (tied at $\alpha = +0.99$\,pp), constitute the remaining top performers, all sharing the common feature of placing equalization before normalization. The worst-performing pipeline is E$\to$D$\to$N$\to$CS at $\alpha = -5.30$\,pp ($\alpha_w = -5.95$), followed by N$\to$D$\to$CS$\to$E at $\alpha = -3.07$\,pp.

Single-transform pipelines are surprisingly competitive when computational cost is considered. Equalization alone achieves $\alpha = +0.83$\,pp with $\alpha_w = 1.00$, and denoising achieves $\alpha = +0.66$\,pp with $\alpha_w = 0.82$, cost-adjusted values that match or exceed those of most multi-stage pipelines. Among length-two pipelines, E$\to$D and E$\to$N stand out with $\alpha = +0.83$\,pp and $+0.58$\,pp respectively, indicating that simple two-step sequences capture most of the achievable benefit at minimal computational cost.

A striking pattern emerges among the worst-performing pipelines: the five most detrimental configurations all violate the equalization-first, normalization-last heuristic developed in Section~IV-D, and four of the five place normalization in a non-terminal position. This systematic clustering of failure around specific positional violations motivates the positional analyses presented in Sections~IV-C and~IV-D.
...
CS                & $-0.33$ & $-0.26$ & CS$\to$D             & $-1.57$ & $-1.28$ \\
D                & $+0.66$ & $0.82$  & CS$\to$E             & $+0.25$ & $0.30$ \\
E                & $+0.83$ & $1.00$  & CS$\to$N             & $-2.49$ & $-1.90$ \\
N                & $+0.08$ & $0.11$  & D$\to$CS             & $-1.74$ & $-1.42$ \\
                &     &     & D$\to$E             & $+0.58$ & $0.53$ \\
                &     &     & D$\to$N             & $-0.33$ & $-0.27$ \\
                &     &     & E$\to$CS             & $+0.50$ & $0.59$ \\
                &     &     & E$\to$D             & $+0.83$ & $0.75$ \\
...
\midrule
% --- Length 3 ---
CS$\to$D$\to$E          & $-1.74$ & $-1.95$ & E$\to$CS$\to$D          & $+0.08$ & $0.07$ \\
CS$\to$D$\to$N          & $-1.74$ & $-1.41$ & E$\to$CS$\to$N          & $-2.40$ & $-2.06$ \\
CS$\to$E$\to$D          & $-2.73$ & $-3.06$ & E$\to$D$\to$CS          & $-0.91$ & $-1.02$ \\
CS$\to$E$\to$N          & $+1.57$ & $1.86$  & E$\to$D$\to$N          & $+0.99$ & $0.89$ \\
CS$\to$N$\to$D          & $-1.74$ & $-1.42$ & E$\to$N$\to$CS          & $-0.41$ & $-0.35$ \\
CS$\to$N$\to$E          & $-1.16$ & $-1.02$ & E$\to$N$\to$D          & $+1.16$ & $1.05$ \\
D$\to$CS$\to$E          & $-0.91$ & $-1.02$ & N$\to$CS$\to$D          & $-0.99$ & $-0.82$ \\
D$\to$CS$\to$N          & $-1.99$ & $-1.63$ & N$\to$CS$\to$E          & $-2.40$ & $-2.08$ \\
D$\to$E$\to$CS          & $-2.32$ & $-2.60$ & N$\to$D$\to$CS          & $-1.33$ & $-1.08$ \\
D$\to$E$\to$N          & $+0.99$ & $0.90$  & N$\to$D$\to$E          & $-1.08$ & $-1.21$ \\
D$\to$N$\to$CS          & $-2.49$ & $-2.03$ & N$\to$E$\to$CS          & $-2.65$ & $-2.28$ \\
D$\to$N$\to$E          & $-1.74$ & $-1.96$ & N$\to$E$\to$D          & $-1.24$ & $-1.40$ \\
\midrule
% --- Length 4 ---
CS$\to$D$\to$E$\to$N       & $-1.33$ & $-1.49$ & E$\to$CS$\to$D$\to$N       & $+0.50$ & $0.44$ \\
CS$\to$D$\to$N$\to$E       & $-1.66$ & $-1.90$ & E$\to$CS$\to$N$\to$D       & $+0.17$ & $0.15$ \\
CS$\to$E$\to$D$\to$N       & $-0.91$ & $-1.03$ & E$\to$D$\to$CS$\to$N       & $-0.41$ & $-0.46$ \\
CS$\to$E$\to$N$\to$D       & $-0.91$ & $-1.01$ & E$\to$D$\to$N$\to$CS       & $-5.30$ & $-5.95$ \\
CS$\to$N$\to$D$\to$E       & $-2.15$ & $-2.47$ & E$\to$N$\to$CS$\to$D       & $-2.24$ & $-2.51$ \\
CS$\to$N$\to$E$\to$D       & $-2.24$ & $-2.55$ & E$\to$N$\to$D$\to$CS       & $-2.32$ & $-2.61$ \\
D$\to$CS$\to$E$\to$N       & $-1.08$ & $-1.21$ & N$\to$CS$\to$D$\to$E       & $-2.24$ & $-2.53$ \\
D$\to$CS$\to$N$\to$E       & $-2.90$ & $-3.32$ & N$\to$CS$\to$E$\to$D       & $-2.07$ & $-2.36$ \\
D$\to$E$\to$CS$\to$N       & $-0.58$ & $-0.65$ & N$\to$D$\to$CS$\to$E       & $-3.07$ & $-3.49$ \\
D$\to$E$\to$N$\to$CS       & $-0.41$ & $-0.46$ & N$\to$D$\to$E$\to$CS       & $-2.24$ & $-2.54$ \\
D$\to$N$\to$CS$\to$E       & $-1.41$ & $-1.61$ & N$\to$E$\to$CS$\to$D       & $-0.99$ & $-1.14$ \\
D$\to$N$\to$E$\to$CS       & $-2.49$ & $-2.83$ & N$\to$E$\to$D$\to$CS       & $-1.99$ & $-2.27$ \\
\bottomrule
\end{tabular*}
\end{table*}
...
At pipeline length~4, all 24 pipelines share the same transform set $\{\text{E}, \text{N}, \text{D}, \text{CS}\}$, so ordering is the sole source of variance by construction. The within-set standard deviation and range at this length demonstrate that rearranging the same four transforms can swing accuracy by nearly 6 percentage points (from $+0.50$ to $-5.30$\,pp), exceeding the entire range of $\alpha$ across all four single-transform pipelines by a factor of five.
...
Categorizing each pipeline by the direction of its error shift reveals that approximately 67\% of all tested combinations produce a threshold-shift pattern, with false positives and false negatives moving in strictly opposite directions. Of these, 41\% shift the effective decision boundary toward the healthy classification (increasing false negatives while reducing false positives), while 27\% shift it toward the diseased classification (increasing false positives while reducing false negatives). An additional 8\% exhibit partial changes affecting only one error dimension while the other remains at the baseline level. Only 17\% of pipelines exhibit the simultaneous increase in both error types that would indicate genuine bidirectional feature-space degradation, and these pipelines carry the worst mean accuracy penalty ($\overline{\alpha} = -1.8$\,pp), compared to $\overline{\alpha} = -1.3$\,pp for sensitivity-eroding pipelines and $\overline{\alpha} = -1.1$\,pp for specificity-eroding pipelines. The remaining 8\% of pipelines improve both metrics simultaneously, with a corresponding positive $\overline{\alpha} = +0.9$\,pp, confirming that genuine feature enhancement, while rare, does occur within the explored pipeline space.

% --- F. Grad-CAM Visualization ---
\subsection{Grad-CAM Visualization of Ordering Effects}

To provide qualitative evidence that transform ordering alters the spatial structure of learned feature representations, Grad-CAM activation maps were compared for an identical test image processed through two mirror-ordering pipelines: E$\to$N ($\alpha = +0.58$\,pp) and N$\to$E ($\alpha = -1.33$\,pp). These two-transform pipelines constitute the minimal pair for the bookend hypothesis, differing only in the ordering of the same two transforms.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_gradcam.pdf}
\caption{Grad-CAM activation maps for the same dermoscopic image (true label: diseased) under mirror-ordering pipelines E$\to$N and N$\to$E. Panels~(a,\,c): positive-class (diseased) activations. Panels~(b,\,d): negative-class (healthy) activations. The E$\to$N pipeline produces distributed positive-class attention with negligible negative-class activation, while N$\to$E concentrates positive-class attention and introduces structured negative-class activation around the lesion, consistent with the threshold-shift mechanism.}
\label{fig:gradcam}
\end{figure}

Fig.~\ref{fig:gradcam} presents the positive-class (diseased) and negative-class (healthy) Grad-CAM overlays for the same dermoscopic image (true label: diseased, model confidence: 1.0 under both pipelines). Because both models classify this image correctly with maximum confidence, the comparison isolates how ordering alters the \emph{spatial distribution} of learned features rather than the final prediction. In the higher-performing E$\to$N pipeline, positive-class activation is distributed across multiple lesion-adjacent regions (panel~a), while negative-class activation is diffuse and near-zero (panel~b), consistent with an unambiguous classification in which no competing ``healthy'' evidence is present. In the lower-performing N$\to$E pipeline, positive-class activation collapses to a single concentrated hotspot (panel~c), and, critically, negative-class activation forms a pronounced ring structure centered on the lesion (panel~d), indicating that the reversed preprocessing order has altered the spatial feature distribution such that the model detects competing evidence for the healthy class.

This qualitative difference in negative-class activation is consistent with the threshold-shift mechanism identified in Section~IV-E: the N$\to$E ordering does not eliminate discriminative features entirely but introduces competing healthy-class evidence that shifts the effective decision boundary. It should be noted that this comparison examines a single test image; a systematic same-image analysis across all 65 pipeline configurations would be required to establish the generality of this activation pattern. Nonetheless, the structured negative-class activation under N$\to$E, absent under E$\to$N, provides a spatial account of how ordering-induced distortions propagate to the classification layer.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_positional.pdf}
\caption{Mean $\alpha$ by ordinal position for each transform. Equalization (E) improves toward position~1; normalization~(N) improves toward the final position. Denoising~(D) exhibits a near-zero positional gradient; color space conversion~(CS) shows a moderate negative gradient favoring earlier placement.}
